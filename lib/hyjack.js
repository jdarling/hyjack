...
var getSourceFromHooks = function(hooks){
  var wrapperNames = Object.keys(hooks);
  var wrappers = [];
  wrapperNames.forEach(function(name){
    var wrapper = hooks[name];
    ...
    wrappers.push('"'+name+'": {'+properties.join(',')+'}');
  });
  return 'config = {'+wrappers.join(',')+'};';
};
...
var Hyjack = function Hyjack(opts){
  ...
  try{
    if(options.hooks){
      self.reloadConfigSource(getSourceFromHooks(options.hooks));
    }else{
    ...
    if((!options.hooks)&&(!options.noWatcher)){
      self.watchConfig();
    }
  }catch(e){
...
Hyjack.prototype.watchConfig = function Hyjack_watchConfig(){
  var self = this;
  if(self._fileWatcher){
    self.stopWatchingConfig();
  }
  self._fileWatcher=(function(self){
    return function(curr, prev){
      try{
        self.reloadConfig();
        self.emit('hyjack::config::reload', self);
      }catch(e){
        console.error('ERROR: Hyjack.prototype.watchConfig');
        console.error('Config file: '+self._configFile);
        trace_error(e);
      }
    };
  })(self);
  fs.watchFile(self._configFile, self._fileWatcher);
};

Hyjack.prototype.stopWatchingConfig = function Hyjack_stopWatchingConfig(){
  var self = this;
  if(self._fileWatcher){
    fs.unwatchFile(self._configFile, self._fileWatcher);
    self._fileWatcher = false;
  }
};
